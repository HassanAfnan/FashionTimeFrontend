...
import 'package:FashionTime/screens/pages/CallJoin.dart';
import 'package:FashionTime/screens/pages/call/call_screen.dart';
import 'package:FashionTime/screens/pages/call/caller_screen.dart';
...
class _AddCallScreenState extends State<AddCallScreen> {
  ...
  startCall(friendId,friendPic,friendUserName,friendName, friendToken){
    https.get(
      ...
      print("Agora Token "+value.body.toString());
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Start call",style: TextStyle(fontFamily: 'Montserrat'),),
      ),
  ...
  checkIncomingCall(friendId,friendPic,friendUserName,friendName, friendToken,channelName){
    FirebaseFirestore.instance.collection("callRoom").where("users",arrayContainsAny: [name,friendName]).get().then((value){
      if(value.docs.length <= 0){
        ...
        Navigator.push(context,MaterialPageRoute(builder: (context) => CallJoin(),));
      }else {
        // popup
